...
def analysis(frame, neigh_clf, count):
    ...
    for pos, el in enumerate(list_conv_defs_l):
        ...
        for i in range(el.shape[0]):
            ...
            if el.shape[0] == 1:  # Giro izquierda o derecha me da igual
                ...
                mark = False
                hasLine = 1
    #print("he logrado salir")
    if not newcnts_am:
        # print("AQUI HACE RESET ")
        type_aut._reset()

    else:
        # print("paso por aqui 2")
        # print("type----", type_aut.getType(mark))
        if not type_aut.getType(mark):  # Mark
            # print("analizando")
            if len(newcnts_am) == 1:
                hu_mom = cv2.HuMoments(cv2.moments(newcnts_am[0])).flatten()
                pred = neigh_clf.predict([hu_mom])
                if pred == 0:#He quitado la marca de la cruz porque he visto que no hay cruces ni en el "wolrd" ni en las imagenes para el examen final
                    cv2.putText(analy, "", (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
                    #type_aut._reset()
                    ...
                    cv2.putText(analy, "Mujer", (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
                    #type_aut._reset()
            ...
            for c in newcnts_am:
                ...
                #print("Arrow dentro de funcion=", arrow)
                angle360 = angle
                if 45 <= angle <= 135:
    ...
    # print("valor de forward", forward)
    # print("valor de turn", turn)
    # print("Valor de hasLine", hasLine)
    # print("valor de mark",  mark)
    # print("valor real de marca salida ", not mark)
    # print("------------------------------------")
    #print("valor de analy\n", analy)
    return forward, turn, hasLine,  not mark, analy
...
def training():
    orig_img = imread('imagenOrigen.png')
    mark_img = imread('imagenMarcada.png')
